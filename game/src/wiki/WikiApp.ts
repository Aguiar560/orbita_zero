...
import { CHANCE_DROP_CHAVE_REPETICAO, CHAVES_DE_ACESSO } from '@data/chaves-de-acesso';
...
function paginaHistoria(): string {
  return `${cabecalhoPagina('CÂNONE OFICIAL · SPOILERS', 'A Guerra dos Selos', 'A história central de Órbita Zero, da queda da rede Primeva à aliança dos Trinta Domínios.')}
    ...
    <section class="wiki-section wiki-keys"><div class="wiki-section-title"><span>LOGÍSTICA DE ACESSO</span><h2>Uma chave para cada domínio.</h2><p>Chaves caem visualmente dos inimigos nos setores anteriores ao chefe. A primeira conclusão do setor 9, 19, 29 e seguintes garante a chave daquela galáxia; depois, cada conclusão pré-chefe tem ${Math.round(CHANCE_DROP_CHAVE_REPETICAO * 100)}% de chance. A chave é coletada no Armazém, não ocupa espaço do Inventário e só abre o chefe da própria galáxia.</p></div><div class="wiki-key-grid">${CHAVES_DE_ACESSO.map((chave) => `<article style="--accent:${chave.cor}"><img src="/assets/${chave.arte}" alt=""><div><span>GALÁXIA ${String(chave.galaxia + 1).padStart(2, '0')} · ${escapeHtml(getElement(chave.elemento).name)}</span><h3>${escapeHtml(chave.nome)}</h3><p>${escapeHtml(chave.descricao)}</p></div></article>`).join('')}</div></section>
